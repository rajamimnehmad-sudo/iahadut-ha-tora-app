...
(async () => {
  ...
  function renderMore() {
    ...
    const officialWebsite = `<a class="more-row official-site-row" href="https://vaad.ar/" target="_blank" rel="noopener"><span class="more-row-leading-icon" aria-hidden="true">↗</span><span><strong>Sitio web oficial</strong><small>Visitá vaad.ar para consultar toda la información</small></span><span class="row-arrow" aria-hidden="true">›</span></a>`;
    ...
    $('#moreList').innerHTML = saved + officialWebsite + Object.entries(info).map(([key, value]) => `<button class="more-row" data-info="${key}">${infoIcon(key)}<span><strong>${escapeHtml(value[0])}</strong><small>${escapeHtml(value[1])}</small></span><span class="row-arrow" aria-hidden="true">›</span></button>`).join('') + update + developerCredit;
